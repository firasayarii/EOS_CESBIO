...
# Standardisation
cols_to_scale = data.drop(columns=['alpha']).columns
scaler = StandardScaler()
scaler_y=StandardScaler()
X_scaled = scaler.fit_transform(data[cols_to_scale])
y_scaled=scaler_y.fit_transform(data['alpha'].values.reshape(-1, 1))
# Encodage one-hot
...
joblib.dump(scaler, 'scaler_SS_DART.pkl')
joblib.dump(scaler_y, 'scaler_y_SS_DART.pkl')
